<section id="formulario">
    ...
    <?php if (count($fotos) > 0) { ?>
        ...
        <form class="padrao galeria" id="formFotos" name="formFotos" method="post" action="<?php echo url::base() ?>galeria/update" >
                        ...
                        if ($img) {
                            ...
                            if (window.confirm('Deseja realmente excluir essa imagem?')) {
                                location.href = URLBASE + 'galeria/excluir/<?php echo $fot->GAL_ID ?>/<?php echo $informacoes["modulo"] ?>/<?php echo $informacoes["id_item"] ?>';
                                   ...
                                   ">
                                    <img src="<?php echo url::base() ?><?php echo $img[0] ?>" />

                                </a>

                                <input type="hidden" id="GAL_ID" readonly name="GAL_ID[]" value="<?php echo $fot->GAL_ID ?>">

                                <div class="item-form" style="margin-top: 5px;">
                                    <label >Legenda</label>
                                    <input type="text" name="legenda[]" id="legenda" value="<?php echo $fot->GAL_LEGENDA ?>" />
                                </div>

                                <div class="item-form">
                                    <label >Créditos</label>
                                    <input type="text" name="ordem[]" id="ordem" label="Ordem" value="<?php echo $fot->GAL_ORDEM ?>" />
                                </div>
            ...
            <div class="final">
                <button type="submit" class="btn pull-right btn-success" id="salvarFotos">Salvar</button>
            </div>
        </form>
    <?php } ?>
