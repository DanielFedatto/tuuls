...
class Controller_Contatos extends Controller_Index {
    ...
    public function action_index($mensagem = "", $erro = false) {
        ...
        $contatos->setColumns(array("CON_ID"=>"CON_ID", "CON_NOME"=>"CON_NOME", "CON_EMAIL"=>"CON_EMAIL", "CON_TELEFONE"=>"CON_TELEFONE", "CON_DATA"=>"CON_DATA"));
        
        //TESTA SE TEM PESQUISA
        if(isset($_GET["chave"])){
            $contatos = $contatos->where("CON_NOME", "like", "%".$this->sane($_GET["chave"])."%")->or_where("CON_EMAIL", "like", "%".$this->sane($_GET["chave"])."%")->or_where("CON_TELEFONE", "like", "%".$this->sane($_GET["chave"])."%")->or_where("CON_DATA", "like", "%".$this->sane($this->ddmmaaaa_aaaammdd(addslashes($_GET["chave"])))."%");
        }
    ...
    public function action_edit(){
        ...
        if($id){
            ...
            $arr = array(
                ...
                "CON_TELEFONE" => $contatos->CON_TELEFONE,
    ...
    public function action_excluir(){
        $contatos = ORM::factory("contatos", $this->request->param("id"));
            
        //SE CARREGOU O MÓDULO, DELETA. SENÃO, NÃO FAZ NADA
        if ($contatos->loaded()){
            //DELETA
            $query = $contatos->delete();
        }else{
            $query = false;
        }
    ...
    public function action_excluirTodos() {$query = false;
        ...
        foreach ($this->request->post() as $value) {
            foreach($value as $val){
                $contatos = ORM::factory("contatos", $val);
