...
class Controller_Index extends Controller_Template {
    ...
    public function before() {
        ...
        $this->template->face_url = $this->dominio;
        $this->template->face_titulo_pagina = $this->empresa;
        $this->template->face_site_name = $this->empresa;
        $this->template->face_descricao = $this->empresa;
        $this->template->face_imagem = "";
        $this->template->face_type = "";

        $this->template->banners = ORM::factory("banners")->find_all();
    ...
    public function action_index() {
        ...
        $view->linhaprodutos = ORM::factory("linhaprodutos")->where("LIP_ID","=","1")->find_all();
        $view->tuuls = ORM::factory("tuuls")->find_all();
        $view->vantagens = ORM::factory("vantagens")->find_all();
        $view->conversaotuuls = ORM::factory("conversaotuuls")->where("COT_ID","=","1")->find_all();
        $view->conversaotopicos = ORM::factory("conversaotopicos")->find_all();
        $view->redetuuls = ORM::factory("redetuuls")->where("RET_ID","=","1")->find_all();
        $view->blog = ORM::factory("blog")->order_by("BLO_DATA", "DESC")->limit("4")->find_all();
                                ...
                                <table cellpadding="0" cellspacing="0" border="0" align="center" width="700" style="background-color:' . $this->coresEmail['background'] . ';border-radius:35px;">
                                    <tr>
                                        <td style="padding:15px;text-align:center;">
                                            <img src="' . $this->dominio . 'images/logo_tuuls.png" width="260" style="vertical-align:middle;"/>
    ...
    public function action_franqueado() {
        $this->auto_render = FALSE;
        ...
        if ($this->request->is_ajax()) {
            ...
            //TENTA SALVAR. SE NÃO PASSAR NA VALIDAÇÃO, VAI PRO CATCH
            try{
                $sejaumfranqueado->SEF_DATA_CADASTRO = date('Y-m-d');
                            ...
                            <tr>
                                ...
                                <table cellpadding="0" cellspacing="0" border="0" align="center" width="700" style="background-color:' . $this->coresEmail['background'] . ';border-radius:35px;">
                                    <tr>
                                        <td style="padding:15px;text-align:center;">
                                            <img src="' . $this->dominio . 'images/logo_tuuls.png" width="260" style="vertical-align:middle;"/>
                                            <span style="color:' . $this->coresEmail['basecolor'] . ';margin-left:150px;font-weight:bold;font-size:20px;">Seja um Franqueado</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td style="padding:5px;">
                                            <div style="background-color:#fff;margin:auto;padding:15px;width:660px;border-radius:10px;">
                                                <h1 style="color:' . $this->coresEmail['basecolor'] . ';margin-top:0;margin-bottom: 15px;">Seja um Franqueado</h1>
                                                <p style="line-height:1.6em;margin:10px 0;">' . $texto . '</p>
                                            </div>
                                        </td>
                                    </tr>
                ...
                // Defina o email e o nome que aparecerá como remetente no cabeçalho
                $mail->From = $query->SEF_EMAIL;
                $mail->FromName = $query->SEF_NOME;
                ...
                // Assunto da mensagem
                $mail->Subject = "Seja um Franqueado";
